...
int32_t rsi_driver_wlan_send_cmd(rsi_wlan_cmd_request_t cmd, rsi_pkt_t *pkt)
{
  ...
  switch (cmd) {
    ...
    case RSI_WLAN_REQ_SET_REGION: {
      ...
      rsi_set_region->region_code = WifiAPSet.AP_COUNTRY;
    ...
    case RSI_WLAN_REQ_EAP_CONFIG: {
      ...
      // EAP 설정 반영
      rsi_strcpy((int8_t *)rsi_eap_req->eap_method, wifiEapSet.eapMethod);      // Method (TLS, TTLS, PEAP...)
      rsi_strcpy((int8_t *)rsi_eap_req->inner_method, "\"auth=MSCHAPV2\"");
      rsi_uint32_to_4bytes((uint8_t *)rsi_eap_req->okc_enable, OKC_VALUE);
      rsi_strcpy((int8_t *)rsi_eap_req->private_key_password, "");
