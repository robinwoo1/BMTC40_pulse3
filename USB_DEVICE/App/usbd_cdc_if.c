...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  /*******************************************************************************/
    case CDC_SET_LINE_CODING:
       LineConding.bitrate    = (uint32_t)((pbuf[3]<<24) | (pbuf[2]<<16) | (pbuf[1]<<8) |(pbuf[0]));
       LineConding.format     = pbuf[4];
       LineConding.paritytype = pbuf[5];
       LineConding.datatype   = pbuf[6];
    break;
...
uint8_t CDC_Transmit_FS(uint8_t* Buf, uint16_t Len)
{
  ...
  {
  	if(HAL_GetTick() - Timeout > 20 && HAL_GetTick() > 2)
		  break;
  }            // waitting send complete   01 07 c0 2c
